...
(deffunction JUEGO::print_tablero (?blancas ?negras)
    ...
    (while (> ?v 0)
        (loop-for-count (?i 1 ?*DIM*)
            (bind ?taero (str-cat ?taero (str-cat "N" ?v ?i) " ")))
            (bind ?v (- ?v 1))
    )
    (bind ?taero (explode$ ?taero))
(loop-for-count(?a 1 ?longnegras)
(bind ?pos (member$ (nth$ ?a ?negras) ?taero))
(bind ?taero (replace$ ?taero ?pos ?pos "X"))
)
(loop-for-count(?a 1 ?longblancas)
(bind ?pos (member$ (nth$ ?a ?blancas) ?taero))
(bind ?taero (replace$ ?taero ?pos ?pos "O"))
)
(bind ?longtab (length$ ?taero))
(bind ?m ?*DIM*)
(bind ?h 1)
(while (> ?m 0)
    (loop-for-count (?j 1 ?*DIM*)
        (bind ?var (nth$ ?h ?taero))
        (if (eq ?var "X") then
        (printout t "X ")
        else
        (if(eq ?var "O") then
