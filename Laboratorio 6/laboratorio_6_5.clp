...
(defrule R1
    ...
    (actividad (nombre ?v) (ciudad ?c) (duracion ?d1))
    ?prev <- (actividades ?n ?c ?d2 ?val)
    =>
    (assert (actividades ?n ?c (+ ?d2 ?d1) (+ ?val 1)))
    (retract ?prev)
)
...
(defrule R2
    (declare (salience 10))
    (persona (nombre ?n) (ciudad ?c))
    (actividad (nombre ?v) (ciudad ?c) (duracion ?d))
    =>
    (assert (actividades ?n ?c ?d 0))
)
